...
|---:|---:|---|---|
| 0 | 2 | 帧头 | `0xFF 0xAA` |
| 2 | 2 | 长度 | 手册标称 `0x1807`，表示 6151；线上字节顺序由 P0 fixture 固化 |
| 4 | 1 | 功能码 | `0x01` |
| 5 | 6144 | 内容 | 3072 个小端 `uint16` |
| 6149 | 1 | CheckSum | 单字节补码校验 |
| 6150 | 1 | 帧尾 | `0xFA` |

内容从左上角开始，从左到右逐行排列。解码值执行：

```text
raw_u16 = low_byte | (high_byte << 8)
sensor_count = raw_u16 & 0x0FFF
matrix = sensor_count.reshape(48, 64)
```
...
等价结果为输入字节求和后的二补数低 8 位。手册没有明确调用时传入的起止范围，因此发布版不得猜测覆盖范围。P0 真机 golden fixture 必须确定覆盖范围，并将其固化为 `protocol_profile_version`；解析器允许在受控协议配置中选择已经 fixture 验证的范围，未知范围时不得把会话标记为有效。
...
1. 接受任意大小、任意边界的字节块；
2. 验证帧头、长度、功能码、CheckSum 和帧尾；
3. 校验失败时从下一个候选 `0xFF 0xAA` 重新同步，不清空全部缓冲；
...
清单包含分段数量、总帧数、总字节数、所有分段摘要、结束时间、版本和本地质量结论。服务端重新计算集合并检查授权、租户和设备上下文。
...
- golden fixture 覆盖真实帧和已确认 CheckSum 范围；
- 逐字节、随机分块、粘包、半帧、噪声和错误帧尾；
- 缓冲有界，错误后能重新同步；
- 48×64 顺序、12-bit 屏蔽和主机时间字段一致；
- 30 分钟真机测试记录帧间隔和校验统计。
...
2. DO-P4864 CheckSum 覆盖范围已由真机 fixture 确认；
