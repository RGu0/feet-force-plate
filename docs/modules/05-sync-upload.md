...
如果第 2 步返回同一机构编号的另一云端受试者 UUID，队列必须进入 `CONFLICT`，不能把原同意改绑或继续第 3 步。经受控核对后的逐会话恢复记录单独加密持久化；队列重新查询云端映射并验证原信封摘要，再上传新签名同意，以云端 UUID 和新同意 ID 创建原会话。原信封、分段与原同意保持可审计且不可变。恢复记录缺失、映射变化或同意被撤回时继续阻断。
