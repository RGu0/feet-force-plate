...
- 本地质量门控和重新检测；
- 本地基础报告；
- 原始数据加密分段、完整 `VALID` 会话的原子网络交接和断点续传；
- 云端原始数据存储、一级特征和完整报告；
- 报告预览、PDF 导出和打印；
- 自动故障日志、设备健康和诊断包兜底。
...
| FR-302 | 分段可靠落盘、校验并 `SEALED` 为本地不可变原始证据；不得因单个分段关闭而创建上传任务 | P0 |
| FR-303 | 仅完整且质量为 `VALID` 的会话在不可变正式晋升时，原子登记全部原始分段和持久化 `READY_FOR_NETWORK` handoff；上传不得阻塞设备线程 | P0 |
...
| FR-307A | 离线受试者的机构编号已指向云端另一 UUID 时，暂停该会话上传；操作员核对确为同一人并重新取得针对云端档案的同意后，补传原始四段数据，不重测、不改写旧同意 | P0 |

`INVALID`、取消、中断、不完整、缺阶段或晋升失败的会话只保留本地诊断证据，绝不创建 handoff 或上传；`SEALED` 仅表示单个本地文件已安全关闭，并不表示可上传。
