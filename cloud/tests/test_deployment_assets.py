...
def test_release_installer_preflights_before_exact_legacy_cutover() -> None:
    ...
    assert text.index("0010_controlled_identity_recovery.sql") > text.index("-f \"$role_wrapper\"")
    assert text.index("to_regclass('ops.session_holds')") < text.index("0010_controlled_identity_recovery.sql")
