...
| 平台基座 | `techflex-cloud-foundation`（私有 wheel，锁版本 + SHA-256；敏感通信与授权信任链的权威实现，见 §14.1） |
...
- 每次会话绑定当时有效的授权记录版本。
- 离线会话上传发现同租户既有机构编号对应另一云端 UUID 时，原会话信封、原同意和原始分段保持不变；须由已登录操作员核对身份并取得面向云端 UUID 的新签名同意。加密的逐会话恢复授权记录保存原信封摘要、两个 UUID、操作员与新同意；只在同一 SQLite 事务中释放阻断 handoff。上传前再次确认云端编号映射，任何不一致继续阻断。
...
### 14.1 平台基座：Cloud Foundation

敏感通信与授权信任链的权威实现是私有包 `techflex-cloud-foundation`（当前 `0.2.0`，由仓库根的 `foundation-artifact.lock.json` 锁定版本与 SHA-256）。应用只消费它的版本化公开 API，**不得复制或改写其 transport、authorization、credential、trust 与 operation-store 实现**；需要新行为时先在 foundation 仓库发布新版本，再在本仓库提升锁定版本。

| 关注点 | foundation 权威接口 | 应用侧职责 |
|---|---|---|
| 出站 HTTPS | `SecureTransport` | 只提供 base URL、超时与业务载荷，不自建 HTTP client |
| 携带凭据的请求 | `AuthorizedTransport` + `TokenProvider` | 只实现 `current_access_token()` / `refresh()` |
| 凭据存储 | `CredentialVault` | 只实现平台安全存储适配器 |
| 服务器公钥与远程策略 | `TrustBundle` / `SignedTrustBundle` / `TrustBundleVerifier` | 只持有根公钥与已安装 revision |
| License 状态与权益 | `LicenseState` / `LicenseRecord` / `LicenseLifecycle` / `EntitlementDecision` / `EntitlementResolver` | 只实现租户、账号与硬件绑定等业务解析 |
| 可靠操作与重试 | `ReliableOperation` / `OperationStore` / `RetryPolicy` | 上传退避已接入 `RetryPolicy`；存储边界见 §14.3 |
| 审计与指标出口 | `AuditSink` / `MetricsSink` | 只实现具体落地端 |
| 服务端数据库生命周期 | `DatabaseRuntime` / `HealthProbe` / `TransactionScope` | 仓储层自有 SQL 与事务内容 |

`SecureTransport` 的既定安全属性：只接受 HTTPS，默认 `verify=None` 使用系统信任，私有 CA 使用 PEM 字节或经过验证的 `SSLContext`，拒绝关闭证书及主机名校验；`trust_env=False` 禁止 httpx 读取环境代理配置；每个请求都带 `X-Correlation-ID`，调用方显式提供时原样复用，否则由 transport 生成。`AuthorizedTransport` 收到 `401` 后**只**刷新一次凭据并以同一 correlation ID 重放，不做刷新循环。

`TrustBundleVerifier` 使用 Ed25519 根公钥验签，并要求 `revision` 严格大于已安装版本以拒绝回滚；bundle 内含 `signing_keys`、`revoked_key_ids` 与策略开关，是服务器公钥集和远程策略的唯一可信来源。

所有版本部署——本机开发、CI 与便携版发布——都必须先通过锁定的 foundation 工件校验，见 [`README.md`](../README.md)、[便携版发布放行清单](release/windows-portable-release-gate.md)与[通信接口设计文档](通信接口设计文档.md) §15。

### 14.2 安全与隐私要求

- 终端使用设备证书或等价的强设备身份，不使用写死的共享密钥。
- 出站敏感通信一律经由 §14.1 的 foundation transport 建立的 TLS 通道；本地分段、身份字段和云端存储加密。
...
### 14.3 已知待对齐（foundation 采纳缺口）

以下为当前代码与 §14.1 权威接口的实测差异，尚未完成迁移，不得当作已完成：

| 缺口 | 现状 | 权威接口 |
|---|---|---|
| 请求超时 | [通信接口设计文档](通信接口设计文档.md) §12 的按接口超时表 | `SecureTransport` 默认 connect 5 s / read 20 s / write 20 s / pool 5 s |
| 可靠操作存储 | 应用自有 `sync_handoffs` 表（见[数据库设计文档](数据库设计文档.md) §4.9） | `OperationStore` 协议与 `foundation_operations` 参考实现 |
| 凭据与信任链 | 仅 `client/cloud/access_client.py`、`client/sync/persistent_upload.py` 接入 `SecureTransport`；License 与 keyset 仍为应用自建 | `AuthorizedTransport`、`CredentialVault`、`TrustBundleVerifier`、`LicenseLifecycle` |
| 服务端数据库生命周期 | `cloud/` 直接使用 asyncpg 建池 | `DatabaseRuntime` / `HealthProbe` |
| 审计与指标出口 | 观测模块自有出口，未经 foundation 协议 | `AuditSink` / `MetricsSink` |

每项差异在 RAY-428 下各自登记独立的交付 scope 后再收敛；在收敛前，本文档与相关模块文档按现状如实描述，并标注本节。收敛完成的行必须从上表移除。
...
```text
feet-force-plate/
├── dev / dev.ps1               # 唯一受控命令入口（setup/test/lint/build/run）
├── foundation-artifact.lock.json  # techflex-cloud-foundation 版本与 SHA-256 锁定
├── .foundation-artifacts/      # 校验通过的 foundation wheel 缓存（忽略，不提交）
├── main.py
├── client/
│   ├── app/                    # 用例、状态机、依赖组装
│   ├── workflow/               # 一键式机构流程编排
│   ├── device/                 # 串口、DO-P4864 协议、采集
│   ├── hardware_standardization/ # 版本化标准化与设备适配
│   ├── hardware_integration/
│   ├── spool/                  # 本地会话、加密分段、崩溃恢复
│   ├── local_analysis/         # 本地基础算法
│   ├── sync/                   # VALID 会话网络交接与上传
│   ├── cloud/                  # 机构访问与云端客户端
│   ├── security/               # 凭据与密钥封套适配
│   ├── startup_validation/
│   ├── reporting/              # 报告与 PDF
...
│   ├── observability/
│   └── migrations/
├── shared/contracts/           # 端云共享契约
├── tests/                      # 含 tests/foundation/ 版本化消费者契约
├── scripts/
├── deploy/
└── docs/
```

`client/` 与 `cloud/` 均不实现自己的 HTTP client、凭据保管、信任链验签或通用重试存储；这些能力来自 §14.1 的 foundation 公开 API。`tests/foundation/` 固定校验本仓库只依赖该公开 API 及锁定的工件版本。
