...
R8 取代上面的 R6 操作员自行勾选“同一人”流程。客户端仅展示案卷编号、状态和脱敏线索；平台授权人员须在受控边界内使用短时身份访问授权，对机构原始姓名与联系方式两项进行比对。只有服务端返回当前有效的 `MATCHED` 回执，客户端才允许本人或有权代理人重新同意必要处理与补传。算法研究同意单独选择，默认关闭。服务端在一个事务中消耗回执并登记新同意与原会话；回执拒绝后本地保留原始数据和旧授权加密审计记录，等待重新核对。
