...
| 平台基座 | `techflex-cloud-foundation`（公开 GitHub Release wheel，锁版本 + SHA-256；敏感通信与授权信任链的权威实现，见 §14.1） |
...
敏感通信与授权信任链的权威实现是 `techflex-cloud-foundation`（当前 `0.2.0`，由仓库根的 `pyproject.toml` 指定公开 GitHub Release URL，`uv.lock` 锁定版本与 SHA-256）。应用只消费它的版本化公开 API，**不得复制或改写其 transport、authorization、credential、trust 与 operation-store 实现**；需要新行为时先在 foundation 仓库发布新版本，再在本仓库提升锁定版本。
...
├── pyproject.toml              # foundation 版本与 GitHub Release URL
├── uv.lock                     # foundation wheel SHA-256 与项目依赖锁定
├── main.py
├── client/
│   ├── app/                    # 用例、状态机、依赖组装
│   ├── workflow/               # 一键式机构流程编排
