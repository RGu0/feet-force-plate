...
敏感通信与授权信任链的权威实现是私有包 `techflex-cloud-foundation`（当前 `0.1.1`，由仓库根的 `foundation-artifact.lock.json` 锁定版本与 SHA-256）。应用只消费它的版本化公开 API，**不得复制或改写其 transport、authorization、credential、trust 与 operation-store 实现**；需要新行为时先在 foundation 仓库发布新版本，再在本仓库提升锁定版本。
...
| 可靠操作与重试 | `ReliableOperation` / `OperationStore` / `RetryPolicy` | 见 §14.3 已知待对齐 |
| 审计与指标出口 | `AuditSink` / `MetricsSink` | 只实现具体落地端 |
| 服务端数据库生命周期 | `DatabaseRuntime` / `HealthProbe` / `TransactionScope` | 仓储层自有 SQL 与事务内容 |

`SecureTransport` 的既定安全属性：默认 `verify=True`；`trust_env=False`，即忽略环境变量代理与环境 CA，敏感通信不会被本机环境静默改道；每个请求都带 `X-Correlation-ID`，调用方显式提供时原样复用，否则由 transport 生成。`AuthorizedTransport` 收到 `401` 后**只**刷新一次凭据并以同一 correlation ID 重放，不做刷新循环。
...
| 缺口 | 现状 | 权威接口 |
|---|---|---|
| 上传退避 | `client/sync/persistent_upload.py` 自建 equal-jitter 退避 | `RetryPolicy`：无 jitter，`retry_after` 无条件优先 |
