...
持久化队列从 5 秒开始、上限 900 秒。第 `n` 次尝试的退避上界是
`min(900 s, 5 s × 2^(n-1))`，equal-jitter 在该上界的 50%–100% 间取值；
`Retry-After` 大于本地值时优先。重试预算只限制请求频率，不允许丢弃未确认的原始分段。
容量门槛统计所有状态非 `CLOUD_CONFIRMED` 的 handoff：最多 50 个会话和 2 GiB 分段数据，
而不是只统计当前 `UPLOADING` 任务。
