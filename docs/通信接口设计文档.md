...
| 阵列 | 48 × 64，共 3072 点 |
| 单点数据 | 原始 `uint8` 字节；语义、量纲与标定尚未确认 |
| 当前观测帧率 | 约 20.6 Hz（10 分钟空板结构候选）；不作为设备能力或产品采样率承诺 |
| 当前单帧长度 | 3079 bytes |
| 设备帧序号 | 无 |
| 设备时间戳 | 无 |
| 正式下行命令 | 当前资料未定义 |
...
| 4 | 1 | 功能码 | `0x01` |
| 5 | 3072 | 内容 | 3072 个原始 `uint8` 字节，行优先映射到 48×64 |
| 3077 | 1 | CheckSum 候选 | 记录、计数并产生质量标记；当前不作为丢帧条件 |
| 3078 | 1 | 帧尾 | `0xFA` |

内容从左上角开始，从左到右逐行排列。当前接口解码为：

```text
matrix = uint8(frame[5:3077]).reshape(48, 64)
```

`0x1807`、6,144-byte 内容、`uint16`/12-bit 与偏移 6,150 的帧尾不再是
当前接口定义，也不得被运行时代码用作默认帧长或解析规则。它们只保留为
用户提供的历史协议截图来源；RAY-78 evidence 记录了它与当前真机字节流的
冲突，等待供应商确认或新的匹配 golden fixture。
...
- 真实采集回放覆盖 3079-byte 结构、48×64 `uint8` 行优先映射、任意分块与粘包；
- CheckSum 观察计数与质量标记正确；在确认规则前不因候选不匹配丢帧；
- 逐字节、随机分块、粘包、半帧、噪声和错误帧尾；
- 缓冲有界，错误后能重新同步；
- 48×64 `uint8` 顺序和主机时间字段一致；
