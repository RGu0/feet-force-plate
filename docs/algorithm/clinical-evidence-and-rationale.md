...
- 把其他设备、站姿、时长或人群的绝对 COP 阈值直接复制到本产品；
- 把论文 OR、SMD 或相关系数直接变成产品权重；
- 把不同站距、时长、采样率和人群的临界值混用；
- 在标准物理输入、测量一致性和前瞻性验证未完成时输出跌倒概率或医疗诊断。
...
- 同时保留睁眼基础摆动和闭眼相对变化；
- 文献绝对毫米阈值不直接迁移；即使输入使用相同物理单位，也必须匹配站姿、时长、人群和测量一致性规范。
...
- 左右脚在前均采集，前脚和方向必须记录；
- 主要左右差异可以进入 V1 评分，但必须超过测量一致性规范的重复误差和参考人群阈值；
- 轻微差异不扣分，也不诊断单侧疾病。
...
- 约 20 Hz、20 秒可以作为基础时域指标的可行性依据，但每个硬件适配器仍须通过标准输入契约和重复性验证；
- 左右差异只有超过对应测量一致性规范的最小可检测变化后才能进入评分。
...
1. 各硬件适配器从原始值到 N、mm、mm²、s 的准确性和不确定度；
2. 不同阵列布局输出同一标准物理场景时的跨适配器一致性；
3. 固定左转 90°后仍输出统一身体 AP/ML 坐标的契约验证；
4. 各目标硬件上标准半串联足位的完整采集能力；
5. 标准输入与测量一致性规范下的参考人群分布和左右差异最小可检测变化；
...
1. 完成标准物理输入契约、硬件适配器、跨阵列一致性和 20 秒指标重复性验证；
