...
flowchart LR
    ...
    SQLITE --> UPLOAD["VALID 会话持久化 handoff"]
...
erDiagram
    ...
    SESSIONS ||--|| SYNC_HANDOFFS : authorizes
    SYNC_HANDOFFS ||--o{ SEGMENTS : references
...
| `sessions` | 会话上下文和分离状态 |
| `segments` | 不可变分段元数据 |
| `sync_handoffs` | `VALID` 会话的持久化网络交接；取代旧的逐资源 `upload_tasks` 模型 |
| `reports` | 同一会话的报告业务对象 |
...
upload_status:    LOCAL_ONLY / READY_FOR_NETWORK / UPLOADING / CLOUD_CONFIRMED / CONFLICT / BLOCKED
...
仅完整 VALID 会话的不可变正式晋升：
  全部 SEALED segments → READY_FOR_NETWORK
  sync_handoff: READY_FOR_NETWORK → UPLOADING → CLOUD_CONFIRMED
                                      ↘ RETRY_WAIT
                           异常：CONFLICT / BLOCKED
```

`SEALED` 只表示单个文件关闭、校验和可恢复；它不会创建 `upload_task`，也不能单独进入网络队列。只有最终质量门通过的完整 `VALID` 会话才能在同一 SQLite 事务中登记全量分段和一个 `READY_FOR_NETWORK` handoff。`INVALID`、取消、中断、不完整、缺阶段或晋升失败均无 handoff。`UNIQUE(session_id, segment_index)` 防止重复索引。

### 4.9 `sync_handoffs`（取代旧 `upload_tasks`）

| 字段 | 说明 |
|---|---|
| `session_id` | 一会话一交接，引用已正式晋升的 `VALID` 会话 |
| `manifest_sha256/upload_envelope` | 不可变分段清单摘要和加密的 capture-time 上传快照 |
| `state` | `READY_FOR_NETWORK/UPLOADING/RETRY_WAIT/CLOUD_CONFIRMED/CONFLICT/BLOCKED` |
| `attempt_count/next_attempt_at_ns` | 持久化重试及下一次可租赁时间 |
| `last_error_code` | 最近可安全记录的失败分类 |
| `created_at_ns/cloud_confirmed_at_ns` | 建立与云端确认时间 |

`session_id` 唯一，保证正式会话只形成一个交接；worker 租赁的是 handoff，而不是 `SEALED` 分段。状态成为 `CLOUD_CONFIRMED` 前，本地原始分段仍保留。

`sync_handoffs` 是本应用的业务表，承载会话语义（清单摘要、上传信封、容量门槛统计），
不是 foundation `SqliteOperationStore` 的复制品——后者的 `foundation_operations` 是面向新应用的
参考实现，不承担应用 schema。二者的权威边界是：**状态机与重试语义以 foundation `OperationState`
与 `RetryPolicy` 为准**（本表的 `state` 取值即对应 `READY_FOR_NETWORK/UPLOADING/RETRY_WAIT/
CLOUD_CONFIRMED/CONFLICT/BLOCKED`），**业务字段与事务边界由本表自有**。

> **已知待对齐**：当前 `sync_handoffs` 的租赁与重试由应用代码自行实现，未通过 foundation
> `OperationStore` 协议表达；收敛见 RAY-428 与[总体架构设计](架构设计文档.md) §14.3。
...
WHERE upload_status IN ('READY_FOR_NETWORK', 'UPLOADING', 'CONFLICT', 'BLOCKED');

CREATE UNIQUE INDEX ux_segments_session_index
ON segments(session_id, segment_index);

CREATE INDEX ix_sync_handoffs_due
ON sync_handoffs(state, next_attempt_at_ns, created_at_ns)
WHERE state IN ('READY_FOR_NETWORK', 'RETRY_WAIT');
```
...
→ 原子重命名 → SQLite 登记本地 SEALED

完整会话最终质量门通过后：全部 SEALED 分段不可变正式晋升
→ 单一 SQLite 事务写入 VALID 会话、全部 READY_FOR_NETWORK 分段、加密 upload_envelope 和 sync_handoff
```

文件原子关闭失败不得登记 `SEALED`；单个分段关闭绝不创建 `upload_task` 或网络 handoff。正式晋升数据库提交失败时不得留下部分 handoff，恢复扫描只重建可验证的本地 `SEALED` 引用，仍须完整 `VALID` 会话重新完成原子晋升。
...
客户端启动扫描 SQLite、`.tmp`、已关闭分段和报告工件；可验证临时文件恢复为本地 `SEALED`，不可验证文件隔离；中断的 `ACQUIRING` 会话标记 `INCOMPLETE`，且不创建 handoff。恢复只重新租赁既有、未 `CLOUD_CONFIRMED` 的 `sync_handoffs`；不会把独立 `SEALED` 分段、`INVALID`、取消或不完整会话加入网络队列。
