...
客户端侧的出站实现权威是私有包 `techflex-cloud-foundation`（见[总体架构设计](架构设计文档.md) §14.1）。本文定义线上契约；`SecureTransport` / `AuthorizedTransport` 定义客户端如何发出这些请求。两者冲突时以 foundation 的公开 API 行为为准，应用不得为了迁就本文而自建 HTTP client 或凭据刷新逻辑。
...
8. `foundation-artifact.lock.json` 锁定的 `techflex-cloud-foundation` 版本与 SHA-256 已在 CI 校验通过，且客户端出站路径未绕过 `SecureTransport` / `AuthorizedTransport`。
