...
客户端侧的出站实现权威是从公开 GitHub Release 安装的 `techflex-cloud-foundation`（见[总体架构设计](架构设计文档.md) §14.1）。本文定义线上契约；`SecureTransport` / `AuthorizedTransport` 定义客户端如何发出这些请求。两者冲突时以 foundation 的公开 API 行为为准，应用不得为了迁就本文而自建 HTTP client 或凭据刷新逻辑。
...
| 请求头 | 必填范围 | 说明 |
|---|---|---|
| `Authorization` | 激活后全部接口 | `Bearer <short-lived-token>` 或等价设备凭据；由 foundation `AuthorizedTransport` 从 `TokenProvider` 注入，业务代码不自行拼装 |
| `X-Terminal-ID` | 终端接口 | 纳管终端 UUID；服务端必须与凭据绑定关系交叉验证 |
| `X-Correlation-ID` | 全部接口 | 由 foundation `SecureTransport` 保证存在：调用方显式提供时原样复用，否则 transport 生成 UUID；响应原样返回。401 重试复用同一 ID |
...
会话创建载荷必须显式携带 `client_installation_id`；服务端将其与认证主体和 legacy
`terminal_id` 交叉校验，三者不一致即拒绝，不能由客户端选择安装实例或租户。
...
| 401 | 凭据无效或过期 | 由 `AuthorizedTransport` 刷新一次并以同一 correlation ID 重放，仍失败则进入支持状态；不得实现额外刷新循环 |
...
轮换要求旧凭据仍有效或通过受控恢复流程。安装包中不得包含长期共享密钥。客户端侧的凭据保管接口是 foundation `CredentialVault`，令牌获取与刷新接口是 `TokenProvider`；应用只实现平台安全存储适配器，不自建令牌缓存或刷新调度。吊销终端后，服务端拒绝创建新会话和下载新配置，但允许受控上传已存在的本地数据，具体权限由吊销原因决定并审计。
...
| 阵列 | 48 × 64，共 3072 点 |
| 单点数据 | 原始 `uint8` 字节；硬件层按冻结 V1 曲线生成 `estimated_force_n`，供非医疗筛查计算 |
...
|---:|---:|---|---|
| 0 | 2 | 帧头 | `0xFF 0xAA` |
| 2 | 2 | 长度 | `0x0C 0x07`，按大端解为 3,079；作为完整性硬条件 |
| 4 | 1 | 功能码 | `0x01` |
...
的冲突记录；当前运行时真值来自重复的实际采集，而非相互矛盾的厂商资料。
...
等价结果为输入字节求和后的二补数低 8 位。实测紧凑帧不满足该历史公式，
因此它不是当前帧完整性的判断条件。当前接口使用 `OBSERVE` 策略：每个结构
完整帧都记录 CheckSum 观察结果，候选不匹配时添加
`CHECKSUM_MISMATCH_OBSERVED`，但不得因此丢帧；同时添加
`CHECKSUM_NOT_ENFORCED`，明确这是一项审计而非拒帧规则。

原始 `uint8` 字节的压力语义、量纲与标定状态仍须由独立实验建立；这不影响
本节已经由实测确认的帧边界和字节到阵列坐标映射。
...
1. 接受任意大小、任意边界的字节块；
2. 硬验证帧头、`0x0C07` 大端长度、功能码和帧尾；CheckSum 只记录审计结果；
...
服务端校验终端绑定、显式 `client_installation_id` 与认证主体/legacy `terminal_id` 的一致性、
设备型号、授权引用和模式版本，返回 `RECEIVING`。重复请求不得创建第二个会话。
只有已关闭且本地质量为 `VALID` 的正式会话才能形成上传 handoff；该 handoff 将加密的
capture-time envelope 与提升后的不可变原始分段在同一 SQLite 事务中持久化。`INVALID`、
取消、中断、不完整或部分采集会话不得上传。
...
上表是各业务操作的目标值。实际生效的连接层默认值来自 foundation
`SecureTransport`（connect 5 s / read 20 s / write 20 s / pool 5 s）；调用方需要不同值时，
通过 `SecureTransport(timeout=...)` 显式传入，不得另建 HTTP client 绕过它。

退避的权威实现是 foundation `RetryPolicy`：第 `n` 次尝试的间隔为
`min(cap, base × 2^(n-1))`，本项目取 `base = 5 s`、`cap = 900 s`，**不叠加 jitter**；
服务端给出 `Retry-After` 时**无条件优先**于本地计算值。重试预算只限制请求频率，
不允许丢弃未确认的原始分段。容量门槛统计所有状态非 `CLOUD_CONFIRMED` 的 handoff：
最多 50 个会话和 2 GiB 分段数据，而不是只统计当前 `UPLOADING` 任务。

> **已知待对齐**：`client/sync/persistent_upload.py` 目前自建 equal-jitter 退避
> （在 `min(900 s, 5 s × 2^(n-1))` 的 50%–100% 间取值，且仅在 `Retry-After` 大于本地值时优先），
> 与上述 `RetryPolicy` 语义不一致；上表的按接口超时值也尚未全部通过 `SecureTransport(timeout=...)` 落实。
> 两项收敛见 RAY-428 与[总体架构设计](架构设计文档.md) §14.3。
...
sequenceDiagram
    ...
    C->>C: 仅 CLOSED + VALID 会话：原子持久化加密 envelope、不可变分段和 READY_FOR_NETWORK handoff
    C->>I: GET /sessions/{id}/status（恢复优先）
    alt 已 INGESTED 且 VALID
        I-->>C: status
        C->>C: 标记 CLOUD_CONFIRMED
    else 需要继续
        C->>I: POST /subjects（幂等）
        C->>I: POST /consents（幂等）
        C->>I: POST /sessions（幂等，含 client_installation_id）
    loop 每个本地清单缺失分段
...
sequenceDiagram
    ...
    C->>S: 持续关闭并登记分段
    C->>C: 生成本地基础报告
    C->>I: 网络恢复，先查询 /sessions/{id}/status
    alt 已 INGESTED 且 VALID
        I-->>C: status
        C->>C: 标记 CLOUD_CONFIRMED
    else 仍需恢复
        C->>I: 幂等重放 subjects → consents → session（含 client_installation_id）
        C->>I: 查询已接收分段
        I-->>C: received + missing
        C->>I: 仅补传 missing
        C->>I: 重放相同 complete 请求
        C->>I: 再查询 status；仅 INGESTED + VALID 时确认
    end
```
...
无实体 DO-P4864 时，可在本机使用下列开发工具联调既有串口发现、打开和解析路径：

```bash
./dev run python scripts/run_dop4864_virtual_ch340.py
```

另一个终端启动采集端前，显式设置：

```bash
export FEETFORCEPLATE_DEVELOPMENT_VIRTUAL_CH340=1
```

工具在运行期间创建主机端点 `/tmp/ffp-dop4864-host` 和设备端点
`/tmp/ffp-dop4864-device`。应用层发现会将主机端点标记为模拟的通用 CH340
（VID:PID `1A86:7523`），但它不是 macOS USB 枚举设备、没有 USB serial number，不能
作为稳定硬件身份。发生器以 20.7 Hz 写入 `FF AA 0C 07 01`、3,072 个全零 payload
字节、候选 CheckSum `00` 和帧尾 `FA`，总长度 3,079 bytes。

由于 PTY 没有真实串口电气波特率，已启用的模拟主机端点仅在本地以 115,200 打开以兼容
macOS；真实 CH340 保持 1,000,000 baud / 8N1。全零模拟只证明本地开发数据流，不能替代
真实 USB/CH340 识别、物理采集、标定、现场、生产或临床验证。
...
2. 当前 3,079-byte 帧边界、长度、帧头、功能码、帧尾与列优先映射已由实际采集基线确认；CheckSum、原始 `uint8` 语义和标定资料仍按各自独立的物理验证门槛管理；
3. 客户端和服务端契约测试在 CI 中运行；
4. 幂等、摘要冲突、超时、断网和重启测试通过；
5. 租户隔离、终端吊销和凭据轮换测试通过；
6. 字段级日志脱敏和诊断包内容检查通过；
7. 数据库迁移与对象存储补偿流程通过演练。
8. `pyproject.toml` 指定的 `techflex-cloud-foundation` GitHub Release URL 和 `uv.lock` 锁定的版本、SHA-256 已在 CI 校验通过，且客户端出站路径未绕过 `SecureTransport` / `AuthorizedTransport`。

---

## Seed MVP access model

机构认证端点 `/v1/access/activate|login|refresh|logout|license|hardware-lease`
只接受 tenant account 语义；tenant access token audience 为
`feetforceplate-tenant`。Platform 登录和运营端点使用独立
`feetforceplate-platform` audience，角色为 `PLATFORM_OWNER`、
`PLATFORM_OPERATIONS`、`PLATFORM_SUPPORT`、`PLATFORM_ENGINEER`。

provider-provisioned 激活提交账号、一次性码、密码、hardware binding 身份和
client installation；返回 **15-minute access token**、**30-day idle** /
**180-day absolute** 刷新会话及签名 `license/2`。License 是
**6/12-month License**，允许服务端续约、暂停、恢复、撤销；状态变化后
upload and report access continue，而新测试被限制。客户端支持
**24-hour offline grace**，离线全局租约限制必须向用户明确。

同一 tenant 的账号/License/设备组支持 `1 -> 3 -> 2`，API 不提供客户搜索、
创建或加入机构。身份读取还需要 15 分钟 `SensitiveAccessGrant`。旧
terminal API 是 **legacy terminal compatibility**，不再决定 License 所有权。
IP:7443 is not production；正式接入是 domain + public CA + 443。

## 本机故障联调边界

为验证 RAY-428 的恢复语义，可在 Windows 上将 seed API 限定为回环 TLS 服务。
故障控制需要本机生成的令牌，规则按 HTTP 方法和精确路径、次数及失效时间约束，并
写入脱敏审计。该机制可验证 503、延迟/限速、进程重启、完成响应丢失后的幂等重试和
确认乱序；不接受生产凭据或生产数据，也不部署到远端 integration endpoint。最终
证据应记录服务端 `INGESTED`/`VALID`、单次 outbox 事件和客户端恢复结果。
运行器 `scripts/local_controlled_integration_lab.py` 以 `bootstrap`、`serve`、`exercise`
三个阶段在同一私有运行树操作。`exercise` 对完成端点使用同一幂等键重试“执行后丢响应”
场景，并并发请求一个延迟确认和一个后发确认，只有后发确认先返回才记录为通过；随后由
令牌保护的本地监督器重启服务。该入口只允许本机回环管理 DSN，且控制客户端显式不使用
系统代理，防止控制流逃出本机边界。
